"""
Build the CIFAR-10 network.  Summary of available functions: 
 # Compute input images and labels for training. If you want to run evaluations, use inputs() instead.
 ...
 trian_op = train(loss, global_step)
"""
...
def inference(images):
    ...
    Returns:
        Logits.
    """
    # Instantiating all variabels using tf.get_variable instead of tf.Variable in order to share variables across multiple GPU training runs.
    # IF you wanna only run this model on a single GPU, you can simply this function by replacing all instances of tf.get_variable() with tf.Variable().
    ...
    # linear layer(WX + b)
    # no softmax here because tf.nn.sparse_softmax_cross_entropy_with_logits accepts the unscaled logits and performs the softmax internally for efficienty.
...
def _add_loss_summaries(total_loss):
    ...
    Generate moving average for all losses and associated summaries for visualizing the performance of the network.
...
def train(total_loss, global_step):
    ...
    Create an optimizer and apply to all trainable variabels. Add moving average for all trainable variables.

    Parameters:
        total_loss: Total loss from loss().
        global_step: Integer Variable countign the numebr of training steps processed.
